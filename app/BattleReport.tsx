...
import type { DamageLog } from '@/utils/calculatePhaseDamage';
...
// Define PhaseLog and RoundResult types based on simulateRound and calculatePhaseDamage
interface PhaseLog {
  phase: string;
...
interface BattlePhaseProps {
  phaseLog: PhaseLog;
  entry: BattleLogEntry & { roundResult: RoundResult };
  yourProps: {
    race: string;
  ...
  battleLog: (BattleLogEntry & { roundResult: RoundResult })[];
  winner: string;
...
const BattlePhase: React.FC<BattlePhaseProps> = ({
  ...
  const yourPhaseAttack = Object.entries((phaseLog as PhaseLog).yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy || {})
    .filter(([, v]) => (v as number) > 0)
    .map(([unit, count]) => {
      const stats = getEffectiveUnitStats(unit, yourProps.race, yourProps.techLevels, yourProps.strategy, true, 1, undefined, phaseLog.yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy, phaseLog.enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy);
      let attackValue = 0;
      ...
      return (count as number) * attackValue;
    })
    .reduce((sum, val) => sum + val, 0);
  const enemyPhaseAttack = Object.entries((phaseLog as PhaseLog).enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy || {})
    .filter(([, v]) => (v as number) > 0)
    .map(([unit, count]) => {
      const stats = getEffectiveUnitStats(unit, enemyProps.race, enemyProps.techLevels, enemyProps.strategy, true, 1, undefined, phaseLog.enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy, phaseLog.yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy);
      let attackValue = 0;
      if (phaseLog.phase === 'range') attackValue = stats.range;
      else if (phaseLog.phase === 'short') attackValue = stats.short;
      else if (phaseLog.phase === 'melee') attackValue = stats.melee;
    ...
    .reduce((sum, val) => sum + val, 0);
  const yourPhaseDefense = Object.entries((phaseLog as PhaseLog).yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy || {})
    .filter(([, v]) => (v as number) > 0)
    .map(([unit, count]) => {
      const stats = getEffectiveUnitStats(unit, yourProps.race, yourProps.techLevels, yourProps.strategy, true, 1, undefined, phaseLog.yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy, phaseLog.enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy);
    ...
    .reduce((sum, val) => sum + val, 0);
  const enemyPhaseDefense = Object.entries((phaseLog as PhaseLog).enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy || {})
    .filter(([, v]) => (v as number) > 0)
    .map(([unit, count]) => {
      const stats = getEffectiveUnitStats(unit, enemyProps.race, enemyProps.techLevels, enemyProps.strategy, true, 1, undefined, phaseLog.enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy, phaseLog.yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy);
      return (count as number) * stats.defense;
  ...
  // Simulate land gain/loss and building gains (Unused variables removed)

  // Format casualties for display (Unused variable removed)

  // Aggregate total losses for both sides (Unused variable removed)

  return (
    <div className="mb-4 p-3 bg-gray-700 rounded border border-gray-600">
      <div className="font-bold text-purple-200 capitalize mb-4 text-center bg-gray-600 p-3 rounded text-xl border border-purple-400 shadow-lg">{String(phaseLog.phase) === 'end' ? 'End Phase' : `${phaseLog.phase} Phase`}</div>
      {/* End Phase Healing Display */}
      {String(phaseLog.phase) === 'end' ? (
        <div className="mb-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <HealingDisplay side="your" healing={phaseLog.yourHealing as Record<string, number>} />
          <HealingDisplay side="enemy" healing={phaseLog.enemyHealing as Record<string, number>} />
        </div>
      ) : (
        <>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="bg-gray-700 p-3 rounded">
                  ...
                  <div className="space-y-1 text-xs">
                    {(Object.entries(phaseLog.yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy || {}) || []).filter(([, v]) => (v as number) > 0).map(([unit, count]) => {
                        const stats = getEffectiveUnitStats(unit, yourProps.race, yourProps.techLevels, yourProps.strategy, true, 1, undefined, phaseLog.yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy, phaseLog.enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy);
                        let attackValue = 0;
                        if (phaseLog.phase === 'range') attackValue = stats.range;
                        else if (phaseLog.phase === 'short') attackValue = stats.short;
            ...
            <div className="bg-gray-800 p-4 rounded">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="bg-gray-700 p-3 rounded">
                  <div className="text-gray-300 font-medium mb-2">Unit Summary:</div>
                  <div className="space-y-1 text-xs">
                    {(Object.entries(phaseLog.enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy || {}) || []).filter(([, v]) => (v as number) > 0).map(([unit, count]) => {
                        const stats = getEffectiveUnitStats(unit, enemyProps.race, enemyProps.techLevels, enemyProps.strategy, true, 1, undefined, phaseLog.enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy, phaseLog.yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy);
                        let attackValue = 0;
                        if (phaseLog.phase === 'range') attackValue = stats.range;
                        else if (phaseLog.phase === 'short') attackValue = stats.short;
              ...
              <div className="bg-gray-800 p-4 rounded">
                ...
                <div className="space-y-3 text-sm">
                  {(Object.entries(phaseLog.yourArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).yourArmy || {}) || []).filter(([, v]) => (v as number) > 0).map(([unit, count]) => {
                      ...
                      const lost = phaseLog.yourLosses[unit] || 0;
                      const survived = (count as number) - lost;
                      const damageEntry = (phaseLog.yourDamageLog || []).find((d: DamageLog) => d.unitName === unit);
                      ...
                      return (
                        <UnitDetail
                          ...
                          side={"your"}
                          phase={phaseLog.phase as string}
                        />
                      );
                    }).filter((x): x is React.JSX.Element => x !== null)}
                </div>
              ...
              <div className="bg-gray-800 p-4 rounded">
                ...
                <div className="space-y-3 text-sm">
                  {(Object.entries(phaseLog.enemyArmyAtStart || (entry as BattleLogEntry & { roundResult: RoundResult }).enemyArmy || {}) || []).filter(([, v]) => (v as number) > 0).map(([unit, count]) => {
                      ...
                      const damageEntry = (phaseLog.enemyDamageLog || []).find((d: DamageLog) => d.unitName === unit);
                      return (
                        <UnitDetail
                          key={unit}
                          unitName={unit}
...
interface BattleRoundProps {
  entry: BattleLogEntry & { roundResult: RoundResult };
  idx: number;
  yourProps: BattlePhaseProps['yourProps'];
  enemyProps: BattlePhaseProps['enemyProps'];
  originalYourArmy: Army;
  originalEnemyArmy: Army;
  battleLog: (BattleLogEntry & { roundResult: RoundResult })[];
  winner: string;
...
const BattleRound: React.FC<BattleRoundProps> = ({
  entry,
  idx,
  yourProps,
  enemyProps,
  ...
  <div key={idx} className="mb-4 p-3 border-b border-gray-700 last:border-b-0 bg-gray-750">
    <div className="font-bold mb-3 text-lg text-center bg-gray-600 p-2 rounded">Round {String(entry.round)}</div>
    {(entry.roundResult?.phaseLogs || []).map((phaseLog: PhaseLog, pIdxNum: number) => (
      <BattlePhase
        key={pIdxNum}
        phaseLog={phaseLog}
        entry={entry}
...
const BattleReport: React.FC<BattleReportProps> = ({
  ...
  const calculateLandAndBuildingLosses = () => {
    ...
    const landLost = Math.floor((Number((enemyKingdomStats as KingdomStats)?.Land) || 0) * lossPercentage);
    const castlesLost = Math.floor((Number((enemyKingdomStats as KingdomStats)?.Castles) || 0) * lossPercentage);
    const peasantsLost = Math.floor((Number((enemyKingdomStats as KingdomStats)?.Peasants) || 0) * lossPercentage);
    ...
    <div className="p-6 bg-gray-800 rounded-lg mb-6">
      ...
      <div>
        ...
        <div className="max-h-[800px] overflow-y-auto border border-gray-700 rounded">
          {(Object.entries(battleOutcome.battleLog) || []).map(([idx, entry], entryIdx: number) => (
            <BattleRound
              key={entryIdx}
              entry={entry}
              idx={entryIdx}
